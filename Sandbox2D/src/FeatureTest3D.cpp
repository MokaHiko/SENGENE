...
void FeatureTest3D::OnAttach()
{
	...
	{
		// cameras
		SGE::Entity camera = m_Scene->CreateEntity("Main Camera", {30, 30, 0});
		...
		// lights
		SGE::Entity dirLight = m_Scene->CreateEntity("Directional Light", { 0.0, 100.0, 0.0});
		...
		auto ironManModel = SGE::Model::CreateModel("assets/models/iron-man/IronMan/IronMan.obj");
		//m_Model = SGE::Model::CreateModel("assets/models/alienanimal/obj/Alien Animal.obj");
		auto treeModel = SGE::Model::CreateModel("assets/models/tree/Lowpoly_tree_sample.obj");
		//auto spiderModel = SGE::Model::CreateModel("assets/models/spiderbot/source/spiderbot2.gltf");
		auto barrelModel = SGE::Model::CreateModel("assets/models/barrel/wine_barrel_01_4k.obj", true);

		SGE::Entity terrain = m_Scene->CreateEntity("Terrain", {0, 0, 0});
		terrain.AddComponent<SGE::MeshRendererComponent>(cubeModel);
		float sideLength = 200;
		terrain.GetComponent<SGE::TransformComponent>().Scale = {sideLength, 1, sideLength};
		...
		for(int z = 0; z < root; z++)
		{
			...
			{
				ctr++;
				name = "tree " + std::to_string(ctr);
				SGE::Entity tree = m_Scene->CreateEntity(name, {x * spacing - (root * spacing / 2), 4.2, z * spacing - (root * spacing / 2)});
				tree.AddComponent<SGE::MeshRendererComponent>(treeModel);
			}
		}

		{
			name = "barrel";
			SGE::Entity  e= m_Scene->CreateEntity(name, {0, 10, 0});
			e.GetComponent<SGE::TransformComponent>().Scale = glm::vec3{50, 50, 50 };
			e.AddComponent<SGE::MeshRendererComponent>(barrelModel);
		}

		{
			name = "iron man";
			SGE::Entity e = m_Scene->CreateEntity(name, {0, 10, -10});
			e.GetComponent<SGE::TransformComponent>().Scale = glm::vec3{ 0.5, 0.5, 0.5 };
			e.AddComponent<SGE::MeshRendererComponent>(ironManModel);
...
void FeatureTest3D::OnEvent(SGE::Event& event)
{
	...
	if(event.GetEventType() == SGE::EventType::WindowResize) 
	{
		...
		dispatcher.Dispatch<SGE::WindowResizeEvent>(std::bind(&FeatureTest3D::OnWindowResize, this, std::placeholders::_1));
	}
}
...
bool FeatureTest3D::OnWindowResize(SGE::WindowResizeEvent& event)
{
	m_Framebuffer->Resize(event.GetWidth(), event.GetHeight());
	m_ViewPortSize = { event.GetWidth(), event.GetHeight() };
	return false;
}
void FeatureTest3D::ShowFileMenuHierarchy()
{
